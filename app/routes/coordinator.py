...
def coordinator_dash():
    ...
    total_volunteers = db.session.query(Registration.user_id).filter(
        Registration.event_id.in_(event_ids)).distinct().count() if event_ids else 0
    total_regs = db.session.query(Registration).filter(
        Registration.event_id.in_(event_ids)).count() if event_ids else 0
    confirmed_regs = db.session.query(Registration).filter(Registration.event_id.in_(
        event_ids), Registration.status.in_(['confirmed', 'completed'])).count() if event_ids else 0
...
def attendance():
    ...
    if selected_event_id:
        registrations = Registration.query.filter_by(
            event_id=selected_event_id).all()
    if request.method == 'POST':
...
def export_events_csv():
    ...
    writer.writerow(['Title', 'Date', 'Location',
                    'Category', 'Slots', 'Registered'])
    for e in events:
        reg_count = Registration.query.filter_by(event_id=e.id).count()
        writer.writerow([e.title, e.date.strftime('%Y-%m-%d'),
                        e.location, e.category, e.slots, reg_count])
